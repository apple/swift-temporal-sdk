...
extension TemporalClient {
    ...
    public func startWorkflow<each Input: Sendable>(
        ...
        try await self.interceptedService.startWorkflow(
            name: name,
            options: options,
            input: repeat each input
        )
...
extension TemporalClient.Interceptor {
    ...
    func signalWithStartWorkflow<each Input>(
        _ input: SignalWithStartWorkflowInput<repeat each Input>
    ) async throws -> UntypedWorkflowHandle {
        try await withInterceptors(
            interceptors,
            input: input,
            call: (any ClientOutboundInterceptor).signalWithStartWorkflow
        ) { input in
            let runID = try await self.workflowService.signalWithStartWorkflow(
                name: input.name,
                options: input.options,
                headers: input.headers,
