...
struct TemporalTraceRecording {
    ...
    ///
    /// Parenting rather than linking is what keeps a workflow in the same trace as the client call that
    /// started it, matching the Go SDK (which sets the span parent from the header) and the .NET SDK
    /// (which attaches the remote context before starting the workflow activity).
