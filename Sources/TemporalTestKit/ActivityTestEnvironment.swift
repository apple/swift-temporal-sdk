...
public func withActivityTestEnvironment<T>(
...
public func withActivityTestEnvironment<T>(
    ...
    cancellationReason: ActivityCancellationReason? = nil,
    _ body: () async throws -> T,
    assertHeartbeatDetails: @Sendable (AsyncStream<[any Sendable]>) async throws -> Void
) async throws -> T {
    let heartbeatDetails = AsyncStream<[any Sendable]>.makeStream()
    ...
    return try await withoutActuallyEscaping(assertHeartbeatDetails) { escapingClosure in
        try await withThrowingTaskGroup { group in
            group.addTask {
                try await escapingClosure(heartbeatDetails.stream)
            }
...
extension ActivityExecutionContext.Info {
    ...
    ///   - heartbeatValues: Previous heartbeat details.
    ...
    public init(
        ...
        let heartbeatDetailsPayloads = try await dataConverter.convertValues(heartbeatDetails)
